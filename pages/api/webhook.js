...
// Establish connnection to Stripe
const stripe = require('stripe')(process.env.STRIPE_SECRET_KEY);

const endpointScret = process.env.STRIPE_SIGNING_SECRET;
...
export default async (req, res) => {
  if (req.method === 'POST') {
    ...
    try {
      event = stripe.webhooks.constructEvent(payload, sig, endpointScret);
    } catch (error) {
      console.log('ERROR', error.message);
      return res.status(400).send(`Webhooks error: ${error.message}`);
    }
    // Hadndl the checkout.session.completed event
